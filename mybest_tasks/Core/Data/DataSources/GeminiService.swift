...
struct GeminiClient {
    /// ゴールからタスクリストを生成
    var generateTasks: (_ goal: String, _ deadline: Date) async throws -> [GoalTask]
    
    /// チャット指示に基づいてゴールを更新（タスクとマイルストーンの生成）
    var updateGoal: (_ currentGoal: Goal, _ instruction: String) async throws -> (tasks: [GoalTask], milestones: [Milestone])
}

extension GeminiClient: DependencyKey {
    static let liveValue: GeminiClient = {
...
// 互換性のための typealias
typealias GeminiService = GeminiClient
